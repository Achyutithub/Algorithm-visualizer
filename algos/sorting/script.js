...
async function maxHeapify(heapSize, index) {
  let largest = index;
  let left = 2 * index + 1;
  let right = 2 * index + 2;

  if (left < heapSize && heights[left] > heights[largest]) {
    largest = left;
  }

  if (right < heapSize && heights[right] > heights[largest]) {
    largest = right;
  }

  if (largest !== index) {
    swap(index, largest);
    draw([index, largest], ['green', 'red']);
    await maxHeapify(heapSize, largest);
  }
}
